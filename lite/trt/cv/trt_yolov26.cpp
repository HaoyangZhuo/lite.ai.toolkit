...
void TRTYoloV26::ensure_image_buffers(std::size_t required_bytes)
{
  if (required_bytes <= image_capacity) return;
  ...
  check_cuda(cudaHostAlloc(reinterpret_cast<void **>(&new_host), required_bytes,
                           cudaHostAllocDefault),
             "Failed to allocate YOLOV26 pinned image staging buffer");
...
void TRTYoloV26::pack_image(const cv::Mat &mat)
{
  if (mat.type() != CV_8UC3)
  ...
  ensure_image_buffers(required_bytes);
